...
public class ListMembersCountPNGetTest extends ApolloBaseTest{
	...
	public void Verify_that_the_response_code_is_200_and_response_is_in_JSON_format() 
	{
		...
		given().header("Authorization", ConfigReader.getConfig("apollo_api_key")).header("X-Ms-Source", ConfigReader.getConfig("apollo_source"))
				...
				when().get(Resources.listMemberByChannelPNGet()).
	...
	@Test(groups = {"apollo_api"})
// Verify that when no authorization key is passed, response code is 401 and error type is InvalidAuthToken
...
// Verify that when correct authorization key and headers are passed, count of subscribed/unsubscribed/pending/not_subscribed member is showing correct in response.
	...
	{		
			String subMemberCount = mPulseDB.getStringData("select count(*) from member_subscription where account_id="
					+ Resources.accid + " and list_id=" + Resources.listid + "and pn_status='ACTIVE'");
			System.out.println("Count of members subscribed to PN from DB in string "+subMemberCount);
			String unSubMemberCount = mPulseDB.getStringData("select count(*) from member_subscription where account_id="
					+ Resources.accid + " and list_id=" + Resources.listid + "and pn_status= 'INACTIVE'");
			String pendingMemberCount = mPulseDB.getStringData("select count(*) from member_subscription where account_id="
					+ Resources.accid + " and list_id=" + Resources.listid + "and pn_status='PENDING'");
			String notSubMemberCount = mPulseDB.getStringData("select count(*) from member_subscription where account_id="
					+ Resources.accid + " and list_id=" + Resources.listid + "and pn_status is null");
			...
			System.out.println("Count of members subscribed to PN from DB is "+subMemCount);
		
			RestAssured.baseURI = Host;
			Response response = given().header("Authorization", ConfigReader.getConfig("apollo_api_key"))
					.header("X-Ms-Source", ConfigReader.getConfig("apollo_source")).header("Content-Type", ConfigReader.getConfig("apollo_content")).
					when().get(Resources.listMemberByChannelPNGet()).then().contentType(ContentType.JSON).extract().response();
			...
			System.out.println("Count of members subscribed to PN from DB is "+subMemCount);
			System.out.println("Count of members subscribed to PN from response is "+resSubMemCount);
			...
			System.out.println("Count of members unsubscribed to PN from DB is "+unSubMemCount);
			System.out.println("Count of members unsubscribed to PN from response is "+resUnSubMemCount);
			...
			System.out.println("Count of members pending to PN from DB is "+pendingMemCount);
			System.out.println("Count of members pending to PN from response is "+resPendingMemCount);
			...
			System.out.println("Count of members pending to PN from DB is "+notSubMemCount);
			System.out.println("Count of members pending to PN from response is "+resNotSubMemCount);
